...
function DoctorDashboard({ user }) {
  const appointments = getAppointments().filter((a) => a.doctorId === user.id);
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
      ...
      <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-2xl p-6 mb-8 text-white">
        <h1 className="text-2xl font-bold mb-1">Xush kelibsiz, {user.fullName}! 👨‍⚕️</h1>
        <p className="opacity-90 text-sm">{user.specialty || 'Shifokor'} – Bugungi jadval</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <StatCard icon="👥" label="Jami bemorlar" value={appointments.length} color="bg-slate-800 border-slate-700" />
        <StatCard icon="⏳" label="Kelgusi qabullar" value={upcoming.length} color="bg-slate-800 border-slate-700" />
        <StatCard icon="✅" label="O'tgan qabullar" value={appointments.length - upcoming.length} color="bg-slate-800 border-slate-700" />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        <Link
          to="/profile"
          className="bg-slate-800 border border-slate-700 rounded-2xl p-6 hover:shadow-md hover:border-emerald-500 transition-all group"
        >
          <div className="w-12 h-12 bg-emerald-500/10 rounded-xl flex items-center justify-center mb-4 group-hover:bg-emerald-500 transition-colors">
          ...
          <p className="text-sm text-slate-400">Shifokor ma'lumotlari va bemorlar jadvali</p>
        </Link>
      </div>
      ...
      {upcoming.length > 0 && (
        <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6">
          <h2 className="font-semibold text-white mb-4">Yaqinlashayotgan bemorlar</h2>
            ...
            {upcoming.slice(0, 3).map((apt) => (
              <div key={apt.id} className="flex items-center gap-4 p-3 bg-slate-700/50 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-white text-sm truncate">{apt.userFullName}</p>
                  <p className="text-xs text-slate-400 truncate">📝 {apt.complaint}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-emerald-400">{apt.date}</p>
